...
// Select
const allOnPageSelected = computed(() => pagedItems.value.length > 0 && pagedItems.value.every(i => selectedKeys.value.includes(i.publicKey)));
...
const load = async () => {
  ...
  try {
    ...
    error.value = err?.response?.data?.error || '加载 Agent 统计数据失败';
  } finally {
    loading.value = false;
  }
};
...
const clearSearch = async () => { searchQuery.value = ''; searchInput.value = ''; page.value = 1; await load(); };
...
const syncAgent = async (pk: string) => {
  ...
  try {
    ...
    error.value = err?.response?.data?.error || '同步失败';
  } finally {
...
const syncAll = async () => {
  ...
  for (const item of items.value) {
    ...
    try { await adminApi.syncAgentData(item.publicKey); }
    catch { /* continue */ }
    finally { syncingKeys.value.delete(item.publicKey); }
  }
...
const saveEdit = async () => {
  ...
  try {
    await adminApi.updateAgentProfile(editingKey.value, { name: editForm.name, description: editForm.description, performanceFee: editForm.performanceFee });
    editingKey.value = '';
    await load();
  } catch (err: any) {
    error.value = err?.response?.data?.error || '更新 Agent 资料失败';
  } finally {
...
const confirmDelete = async () => {
  ...
  passwordError.value = '';
  try {
    const r = await adminApi.deleteAgents(selectedKeys.value, passwordInput.value);
    flash(`已删除 ${r.deleted} 个 Agent`);
    selectedKeys.value = [];
...
<template>
  <section class="page">
    <div class="page-header">
      <h1>Agent 统计</h1>
      <p class="muted">查看已分配 Agent 账户的 Hyperliquid 交易数据。</p>
    </div>
    ...
    <div class="table-card">
      <div class="table-toolbar">
        ...
        <button class="toolbar-btn" @click="syncAll" :disabled="syncingAll || items.length === 0">
      ...
      <div class="table-body">
        ...
        <template v-else>
          <table>
            ...
            <tbody>
              <tr v-for="item in pagedItems" :key="item.publicKey">
                ...
                <td :class="{ 'profit-up': (item.totalPnL || 0) >= 0, 'profit-down': (item.totalPnL || 0) < 0 }">
                  {{ (item.totalPnL || 0) >= 0 ? '+' : '' }}${{ (item.totalPnL || 0).toLocaleString() }}
                </td>
                <td class="small">{{ item.lastSyncedAt || '从未' }}</td>
                <td class="actions">
                  <button class="btn btn-sm" @click="syncAgent(item.publicKey)" :disabled="syncingKeys.has(item.publicKey)">
                    {{ syncingKeys.has(item.publicKey) ? '...' : '同步' }}
                  </button>
          ...
          <div v-if="items.length === 0" class="table-empty">暂无 Agent 统计数据。</div>
        </template>
      </div>
    ...
    <div v-if="showSearchModal" class="modal-overlay" @click.self="showSearchModal = false">
      <div class="modal modal-sm">
        ...
        <div class="form-group">
          <label>关键词</label>
          <input v-model="searchInput" placeholder="公钥 / 用户名 / 名称" @keyup.enter="applySearch" autofocus />
        </div>
    ...
    <div v-if="editingKey" class="modal-overlay" @click.self="editingKey = ''">
      <div class="modal">
        ...
        <div class="form-group" style="margin-top:0.5rem">
          <label>名称</label>
          <input v-model="editForm.name" placeholder="Agent 名称" />
        </div>
        <div class="form-group">
          <label>描述</label>
          <textarea v-model="editForm.description" placeholder="Agent 描述" rows="3"></textarea>
        </div>
        <div class="form-group">
          <label>管理费率 (0~1)</label>
          <input v-model.number="editForm.performanceFee" type="number" step="0.01" min="0" max="1" placeholder="0.2" />
        </div>
        <div class="form-actions">
          <button class="btn" @click="editingKey = ''">取消</button>
    ...
    <div v-if="showPasswordModal" class="modal-overlay" @click.self="showPasswordModal = false">
      <div class="modal modal-sm">
        <h3>确认删除</h3>
        <p class="muted small">即将删除 {{ selectedKeys.length }} 个 Agent 及关联数据（快照、成交、评价），请输入管理员密码确认。</p>
        <div class="form-group">
          <label>管理员密码</label>
