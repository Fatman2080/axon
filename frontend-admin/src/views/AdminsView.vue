...
const resetPassword = async (admin: AdminUser) => {
  const next = window.prompt(`为 ${admin.email} 设置新密码（至少 6 位）：`, '');
  if (!next) return;
  if (next.trim().length < 6) { error.value = '密码长度不足'; return; }
  error.value = '';
  try {
...
const deleteAdmin = async (admin: AdminUser) => {
  if (!window.confirm(`确认删除管理员 ${admin.email}？`)) return;
  error.value = '';
  try {
          ...
          <table>
            ...
            <tbody>
              <tr v-for="item in admins" :key="item.id">
                ...
                <td class="actions">
                  <button class="btn btn-sm" @click="resetPassword(item)">重置密码</button>
                  <button class="btn btn-sm btn-danger" :disabled="item.id === currentAdminId" @click="deleteAdmin(item)">删除</button>
                </td>
              </tr>
            </tbody>
          </table>
          <div v-if="admins.length === 0" class="table-empty">暂无管理员。</div>
        </template>
      </div>
    </div>
    ...
    <div v-if="showCreateModal" class="modal-overlay" @click.self="showCreateModal = false">
      <div class="modal modal-sm">
        ...
        <div class="form-group">
          <label>邮箱</label>
          <input v-model="createForm.email" type="email" placeholder="admin@example.com" />
        </div>
        <div class="form-group">
          <label>名称</label>
          <input v-model="createForm.name" placeholder="管理员名称" />
        </div>
        <div class="form-group">
          <label>密码</label>
          <input v-model="createForm.password" type="password" placeholder="至少 6 个字符" @keyup.enter="createAdmin" />
        </div>
        <div class="form-actions">
          <button class="btn" @click="showCreateModal = false">取消</button>
          <button class="btn btn-primary" @click="createAdmin" :disabled="creating">
            {{ creating ? '创建中...' : '创建' }}
          </button>
        </div>
      </div>
